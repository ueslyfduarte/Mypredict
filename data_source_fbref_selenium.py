...
# Mapeamento de ligas (nome → código FBref e slug para stats)
LEAGUES = {
...
def _init_driver():
    """Inicializa o Chrome em modo headless, usando o chromedriver do sistema."""
    options = Options()
...
def _baixar_com_selenium(url):
    """Baixa uma página usando Selenium e retorna o HTML renderizado."""
    driver = _init_driver()
    try:
        driver.get(url)
...
def obter_classificacao(liga_nome, temporada):
    """Retorna {posição: nome_time} da tabela de classificação do FBref."""
    ...
    # Procura a tabela de classificação (contém colunas 'wins' e 'losses')
    table = None
    ...
    for _, row in df.iterrows():
        try:
            ...
            continue
    if not classif:
        raise ValueError("Classificação vazia.")
    _cache_escrever(chave, classif)
    return classif

def obter_partidas_time(liga_nome, temporada, time):
    """Retorna lista de partidas de um time, com HT e dados básicos."""
    info = LEAGUES.get(liga_nome)
    if not info:
        raise ValueError(f"Liga '{liga_nome}' não suportada.")
    cod = info["cod"]
    ...
    if not table:
        raise ValueError("Tabela de partidas não encontrada.")
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Tenta extrair o placar de intervalo (HT)
            ht_str = ''
...
def obter_stats_time(liga_nome, temporada, time):
    """Retorna estatísticas agregadas (médias) do time na temporada."""
    info = LEAGUES.get(liga_nome)
    if not info:
        return {}
    cod = info["cod"]
    ...
    # Procura a tabela de estatísticas (pode estar dentro de comentário)
    ...
    # Escanteios podem ser None (não disponíveis na tabela standard)
