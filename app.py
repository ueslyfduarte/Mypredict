...
# =========================================================================
# ABA BACKTESTING OFFLINE – USANDO PANDAS (ROBUSTO)
# =========================================================================
elif aba == "📊 Backtesting Offline":
    st.header("📊 Backtesting Walk‑Forward – Leitura Robusta de CSV")
    st.caption("Cole todo o conteúdo do CSV. O sistema detectará automaticamente as colunas HomeTeam, AwayTeam, FTHG, FTAG, etc.")
    ...
    if st.button("▶️ Iniciar Simulação Completa"):
        if not texto_dados.strip():
            ...
            try:
                import io
                # Lê o CSV com pandas (delimiter vírgula)
                ...
                if faltantes:
                    st.error(f"Colunas obrigatórias não encontradas: {faltantes}. Colunas disponíveis: {list(df.columns)}")
                else:
                    st.success(f"CSV lido com sucesso! {len(df)} jogos encontrados.")

                    # Mapeamento de colunas opcionais
                    col_hthg = 'hthg' if 'hthg' in df.columns else None
                    col_htag = 'htag' if 'htag' in df.columns else None
                    col_hs = 'hs' if 'hs' in df.columns else None
                    col_as = 'as' if 'as' in df.columns else None
                    col_hst = 'hst' if 'hst' in df.columns else None
                    col_ast = 'ast' if 'ast' in df.columns else None
                    col_hc = 'hc' if 'hc' in df.columns else None
                    col_ac = 'ac' if 'ac' in df.columns else None
                    ...
                    # Reseta contadores de desempenho
                    ...
                    for idx, row in df.iterrows():
                        ...
                        gols_m = int(row['fthg'])
                        gols_v = int(row['ftag'])
                        gols_ht_m = int(row[col_hthg]) if col_hthg and not pd.isna(row[col_hthg]) else None
                        gols_ht_v = int(row[col_htag]) if col_htag and not pd.isna(row[col_htag]) else None
                        chutes_m = float(row[col_hs]) if col_hs and not pd.isna(row[col_hs]) else None
                        chutes_v = float(row[col_as]) if col_as and not pd.isna(row[col_as]) else None
                        chutes_gol_m = float(row[col_hst]) if col_hst and not pd.isna(row[col_hst]) else None
                        chutes_gol_v = float(row[col_ast]) if col_ast and not pd.isna(row[col_ast]) else None
                        escanteios_m = float(row[col_hc]) if col_hc and not pd.isna(row[col_hc]) else None
                        escanteios_v = float(row[col_ac]) if col_ac and not pd.isna(row[col_ac]) else None

                        # Funções get_stats e update_stats (idênticas às versões anteriores)
